<#
.SYNOPSIS
    Installs Otter on Windows.

.DESCRIPTION
    Downloads the Velopack installer (Otter-win-Setup.exe) for a GitHub release, verifies it against the
...
.PARAMETER Version
    Install a specific version (e.g. 0.3.1) instead of the latest release. A leading "v" is fine.
    Also reads $env:OTTER_VERSION, which is how you pin through the piped one-liner:
        $env:OTTER_VERSION = '0.3.1'; irm .../install.ps1 | iex

.PARAMETER Repo
    owner/name of the GitHub repository to install from. Defaults to ArcticGizmo/otter; override for a
    fork. Also reads $env:OTTER_REPO.
...
.EXAMPLE
    ...
    & ([scriptblock]::Create((irm https://raw.githubusercontent.com/ArcticGizmo/otter/main/install.ps1))) -Version 0.3.1
#>
#Requires -Version 5.1
param(
    [string] $Version = $env:OTTER_VERSION,
    [string] $Repo    = $(if ($env:OTTER_REPO) { $env:OTTER_REPO } else { 'ArcticGizmo/otter' })
)
...
function Install-Otter {
    [CmdletBinding()]
    param([string] $Version, [string] $Repo)

    $ErrorActionPreference = 'Stop'

    $SetupAsset = 'Otter-win-Setup.exe'
    $SumsAsset  = 'SHA256SUMS.txt'

    # --- Preflight -------------------------------------------------------------------------------------
    if ([System.Environment]::OSVersion.Platform -ne 'Win32NT') {
        throw 'Otter is a Windows application and only installs on Windows.'
    }
    if (-not [System.Environment]::Is64BitOperatingSystem) {
        throw 'Otter ships as 64-bit only; this looks like a 32-bit Windows.'
    }
    if ($env:PROCESSOR_ARCHITECTURE -eq 'ARM64') {
        Write-Warning 'ARM64 Windows detected. Otter is x64-only, so it will run under emulation.'
    }
    ...
    # --- Resolve the release ---------------------------------------------------------------------------
    $release = Get-OtterRelease -Repo $Repo -Version $Version
    $tag     = $release.tag_name
    Write-Host "Installing Otter $tag" -ForegroundColor Cyan
    ...
    $work = Join-Path ([System.IO.Path]::GetTempPath()) ("otter-install-" + [guid]::NewGuid().ToString('N').Substring(0, 12))
...
"@
        ...
        # --- Install -----------------------------------------------------------------------------------
        # Velopack's installer needs no admin rights: it installs to %LocalAppData%\Otter, registers the
        ...
        # which includes the tray app we just installed. Since Otter is meant to keep running, the wait never
        ...
        if (-not $proc.WaitForExit(10 * 60 * 1000)) {
            $keepWork = $true
            Write-Warning "The installer is still running after 10 minutes - leaving it to finish on its own. Delete $work once Otter has installed."
            return
        }
        if ($proc.ExitCode -ne 0) {
            throw "The Otter installer exited with code $($proc.ExitCode)."
        }
    }
    finally {
    ...
    Write-Host ''
    Write-Host "Otter $tag is installed and starting in the system tray." -ForegroundColor Green
    Write-Host '  Left-click the tray icon to open settings, right-click for a quick menu.' -ForegroundColor DarkGray
    Write-Host '  Connect your Slack workspace on the Getting started page to finish setup.' -ForegroundColor DarkGray
...
function Get-OtterRelease {
    param([string] $Repo, [string] $Version)

    $headers = @{ 'Accept' = 'application/vnd.github+json'; 'User-Agent' = 'otter-install.ps1' }
...
# makes a ~60 MB download crawl on Windows PowerShell 5.1, and -OutFile there buffers the whole response
...
function Save-File {
    ...
    $req.UserAgent = 'otter-install.ps1'
    $req.Timeout = 60000
...
Install-Otter -Version $Version -Repo $Repo
